...
Syx_Value *syx_special_form_quote(Syx_Eval_Ctx *ctx, Syx_Pair *arguments) {
  UNUSED(ctx);
  return syx_list_next_nullable(&arguments);
}

/** Evaluates value in quasiquote environments. */
Syx_Value *syx_special_form_unquote(Syx_Eval_Ctx *ctx, Syx_Pair *arguments) {
  UNUSED(arguments);
  SYX_EVAL_THROW(ctx, "unquote: completely outside of a quasiquote environment");
}
...
Syx_Value *syx__special_form_make_lambda(Syx_Eval_Ctx *ctx, Syx_Symbol *name_symbol, Syx_Pair *defines, Syx_Pair *forms) {
  ...
  syx_list_for_each(defines, define, &rest_define) {
    ...
    if (define->kind != SYX_VALUE_KIND_PAIR) SYX_EVAL_THROW(ctx, "malformed lambda arguments definitions list");
    if (define->pair->left->kind != SYX_VALUE_KIND_SYMBOL) SYX_EVAL_THROW(ctx, "malformed lambda arguments definitions list");
    if (define->pair->right->kind == SYX_VALUE_KIND_PAIR) {
      if (define->pair->right->pair->right->kind != SYX_VALUE_KIND_NIL) SYX_EVAL_THROW(ctx, "malformed lambda arguments definitions list");
    }
  }
  if (rest_define->kind != SYX_VALUE_KIND_NIL) {
    if (rest_define->kind != SYX_VALUE_KIND_SYMBOL) SYX_EVAL_THROW(ctx, "malformed lambda rest argument");
  }
  syx_string_view name = sv_from_parts(name_symbol->data, name_symbol->count);
...
Syx_Value *syx_special_form_get(Syx_Eval_Ctx *ctx, Syx_Pair *arguments) {
  ...
  if (name_s->kind != SYX_VALUE_KIND_SYMBOL) SYX_EVAL_THROW(ctx, "Symbol expression expected as name");
  Syx_Value *stored = syx_env_lookup_get(ctx, name_s->symbol);
  if (!stored) return syx_value_nil();
  return stored;
}
...
Syx_Value *syx_special_form_let(Syx_Eval_Ctx *ctx, Syx_Pair *arguments) {
  ...
  syx_list_for_each(bindings_src->pair, binding) {
    if (binding->kind != SYX_VALUE_KIND_PAIR) SYX_EVAL_THROW(ctx, "malformed let definition, list expected", body_ctx);
    if (binding->pair->left->kind != SYX_VALUE_KIND_SYMBOL) SYX_EVAL_THROW(ctx, "malformed let definition, symbol as name expected", body_ctx);
    if (binding->pair->right->kind == SYX_VALUE_KIND_PAIR) {
      if (binding->pair->right->pair->right->kind != SYX_VALUE_KIND_NIL) {
        SYX_EVAL_THROW(ctx, "malformed let definition, too long list", body_ctx);
      }
    }
...
Syx_Value *syx_special_form_try(Syx_Eval_Ctx *ctx, Syx_Pair *arguments) {
  ...
  Syx_Value *result = NULL;
  syx_list_for_each(arguments, branch) {
    if (branch->kind != SYX_VALUE_KIND_PAIR) SYX_EVAL_THROW(ctx, "malformed try handlers, list expected");
    if (branch->pair->left == catch_symbol) {
      if (body->kind != SYX_VALUE_KIND_EXIT && body->exit->kind != SYX_EXIT_KIND_THROWN) continue;
      Syx_Value *list = branch->pair->right;
      if (list->kind == SYX_VALUE_KIND_NIL) {
        if (result) rc_release(result);
        result = rc_acquire(syx_value_nil());
        continue;
      }
      if (list->kind != SYX_VALUE_KIND_PAIR) SYX_EVAL_THROW(ctx, "malformed try's catch, list expected");
      Syx_Value *handler = list->pair->right;
      if (handler->kind != SYX_VALUE_KIND_PAIR) SYX_EVAL_THROW(ctx, "malformed try's catch handler, list expected");
      Syx_Value *error_name = list->pair->left;
      if (error_name->kind != SYX_VALUE_KIND_SYMBOL) {
        if (result) rc_release(result);
        result = rc_acquire(syx_eval_forms_list(ctx, list->pair, .initial = syx_value_nil()));
        syx_value_early_exit(result, catch_symbol, finally_symbol, body);
        continue;
      }
      ...
      if (result) rc_release(result);
      result = rc_acquire(syx_eval_forms_list(handler_ctx, handler->pair, .initial = syx_value_nil()));
      syx_value_early_exit(result, body, handler_ctx, catch_symbol, finally_symbol);
      rc_release(handler_ctx);
      continue;
    }
    if (branch->pair->left == finally_symbol) {
      Syx_Value *list = branch->pair->right;
      if (list->kind != SYX_VALUE_KIND_PAIR) SYX_EVAL_THROW(ctx, "malformed try's finally handler, list expected");
      Syx_Value *finally_result = rc_acquire(syx_eval_forms_list(ctx, list->pair));
      syx_value_early_exit(finally_result, catch_symbol, finally_symbol, body, result);
      rc_release(finally_result);
      continue;
    }
    SYX_EVAL_THROW(ctx, "malformed try's handlers list");
  }
...
void syx_env_define_special_forms(Syx_Env *env) {
  /** Special forms */
  syx_env_define_cstr(env, "quote", make_syx_value_closure_specialf(SVLIT("quote"), syx_special_form_quote));
  syx_env_define_cstr(env, "unquote", make_syx_value_closure_specialf(SVLIT("unquote"), syx_special_form_unquote));
