...
SyxV *syx_eval_unquote(Syx_Eval_Ctx *ctx, SyxV *unevaluated);
SyxV *syx_eval_list(Syx_Eval_Ctx *ctx, SyxV *list);
...
#define syx_eval_early_exit_temporary(value, ...) \
  do {                                            \
    SyxV *_value = (value);                       \
    if (_value) {                                 \
      if (syx_eval_should_early_exit(_value)) {   \
...
SyxV *syx_eval_closure(Syx_Eval_Ctx *ctx, Syx_Closure *closure, SyxV *arguments) {
  ...
  syxv_list_for_each(name_v, closure->defines, &last_name) {
    ...
    SyxV *value = rc_acquire(syx_eval(ctx, it->pair.left));
    syx_eval_early_exit(value, call_ctx);
    syx_env_define(call_ctx->env, symbol, rc_move(value));
    it = it->pair.right;
  }
  if (last_name->kind != SYXV_KIND_NIL) {
    ...
    //   *argument =rc_acquire( syx_eval(ctx, *argument));
...
SyxV *syx_eval(Syx_Eval_Ctx *ctx, SyxV *input) {
  ...
  switch (head->kind) {
    ...
    case SYXV_KIND_BOXED: result = syx_eval_boxed(ctx, head->boxed, arguments); break;
    case SYXV_KIND_BOXED_METHOD: result = syx_eval_boxed_method(ctx, head->boxed_method, arguments); break;
...
SyxV *syx_eval_unquote(Syx_Eval_Ctx *ctx, SyxV *unevaluated) {
  SyxV *unquote_symbol = rc_acquire(make_syxv_symbol_cstr("unquote"));
  if (unevaluated->kind != SYXV_KIND_PAIR) return unevaluated;
  if (unevaluated->pair.left != unquote_symbol) return unevaluated;
  if (unevaluated->pair.right->kind != SYXV_KIND_PAIR) return unevaluated;
  return syx_eval(ctx, unevaluated->pair.right->pair.left);
}

SyxV *syx_eval_list(Syx_Eval_Ctx *ctx, SyxV *list) {
  SyxV *evaluated = NULL;
  syxv_list_map(item, list, &evaluated) {
...
SyxV *syx_convert_to_bool(Syx_Eval_Ctx *ctx, SyxV *value) {
  switch (value->kind) {
    ...
    case SYXV_KIND_BOXED_METHOD: return make_syxv_bool(true);
...
SyxV *syx_convert_to_number(Syx_Eval_Ctx *ctx, SyxV *value) {
  switch (value->kind) {
    ...
    case SYXV_KIND_BOXED_METHOD: RUNTIME_ERROR(ctx, "illegal conversion of boxed value method to number");
